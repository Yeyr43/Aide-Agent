...
├── commands/            # 命令系统
│   └── builtin/
│       ├── handlers.py  # 9 个内置命令（/help /profile /compact /export /import /plugin /session /memory /tools）
│       ├── plugin_commands.py  # /plugin 指令实现
...
- **12 个内置命令**：`/help` `/profile` `/compact` `/export` `/import` `/plugin` `/session` `/memory` `/tools` `/update` `/clear` `/rollback` `/mcp`
