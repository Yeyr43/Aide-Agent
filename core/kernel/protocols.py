...
class ExecutorUI(Protocol):
    ...
    def on_thinking_token(self, token: str) -> None:
        """深度思考 token（Anthropic extended thinking）。
        UI 层用 dimmed 样式渲染，区分于普通文本。"""
        ...
    ...
    def on_tool_blocked(self, tool_name: str, reason: str) -> None:
        """高风险工具被阻止时通知 UI。"""
        ...
...
class NullUI:
    """静默 ExecutorUI — 子 agent 使用，所有回调为空操作。

    delegate 工具派生子 agent 时传入此对象，使子 agent 的流式输出、
    工具调用过程全部静默，不打扰主对话 UI。
    """
