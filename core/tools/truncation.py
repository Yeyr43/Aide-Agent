...
def truncate_output(
    ...
    *,
    ...
    Args:
        text: 要截断的文本
        max_size: 上限（字符数或字节数，取决于 unit）。必须 > 0。
        unit: "chars"（字符数）或 "bytes"（UTF-8 字节数）
        label: 截断标记的自定义文本（可选，默认使用 i18n 标记）
    ...
    tail_size = max(max_size - head_size, 0)
    marker = label or t("tool.truncation.label")

    if unit == "bytes":
        # 字节模式：按字节边界切片，避免多字节字符被截断
        raw = text.encode("utf-8")
        head = raw[:head_size].decode("utf-8", errors="replace")
        tail = raw[-tail_size:].decode("utf-8", errors="replace") if tail_size > 0 else ""
    else:
        head = text[:head_size]
