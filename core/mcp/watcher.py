"""FileWatcher — 可插拔后端的热加载目录监听器。

默认使用轮询后端（零依赖，5s 间隔），可通过 backend 参数替换为
事件驱动后端（如 watchfiles）。对 MCP 配置目录场景，轮询完全够用；
高频变更场景可安装 watchfiles 获得 inotify/ReadDirectoryChanges 级效率。
...
class PollingBackend:
    """mtime 轮询后端 — 定时扫描目录检测 .json 文件变更。

    适合低频变更场景（如配置目录），间隔可配置。
    """
