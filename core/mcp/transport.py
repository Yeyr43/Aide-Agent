...
class HTTPTransport:
    ...
    复用持久 httpx.AsyncClient（连接池、keep-alive）。
    ...
    def __init__(self) -> None:
        ...
        self._request_id = 0
        self._connected = False
        self._client: Any = None  # httpx.AsyncClient（延迟创建）

    def _ensure_client(self) -> None:
        """确保 httpx client 已创建（延迟导入 httpx）。"""
    ...
    async def _http_post(
        ...
        """发送 JSON-RPC 请求并解析响应（复用持久 httpx client）。"""
        import httpx
        ...
        return self._parse_http_response(request.id, resp)

    def _parse_http_response(
        self, request_id: int, resp: Any,
    ) -> JSONRPCResponse:
        """解析 HTTP 响应体，支持纯 JSON 和 SSE (text/event-stream) 两种格式。

        对 SSE 格式提取所有 data: 行，合并后解析最终 JSON-RPC 响应。
        """
        ...
        # SSE 格式：提取所有 data: 行
        if "text/event-stream" in content_type or data.startswith("event:") or data.startswith("data:"):
            data_lines: list[str] = []
            ...
            if data_lines:
                # 多事件 SSE：每行是独立 JSON，取最后一条有效的 JSON-RPC 响应
                if len(data_lines) > 1:
                    ...
                    # 全部无法解析，返回第一条的错误
                    try:
                        return parse_response(data_lines[-1])
                    except (json.JSONDecodeError, Exception):
                        pass
                else:
                    data = data_lines[0]
            else:
                # content-type 是 SSE 但没有 data: 行 → 尝试按纯 JSON 解析
                if data.startswith("{"):
                    pass  # fall through to JSON parse below
                else:
                    return JSONRPCResponse(id=request_id, result={})
    ...
    async def _http_post_notification(
        ...
        """发送 JSON-RPC 通知（不等待响应，复用持久 client）。"""
        import httpx
